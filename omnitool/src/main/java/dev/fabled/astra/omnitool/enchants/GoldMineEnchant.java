        ...
        for (int x = -radius; x <= radius; x++) {
            for (int z = -radius; z <= radius; z++) {
                ...
                if (isPlayerMap(player, currentBlock) && currentBlock.hasMetadata("material")) {
                    Material goldBlockMaterial = Material.GOLD_BLOCK;
                    currentBlock.removeMetadata("material", Astra.getPlugin());
                    currentBlock.setMetadata("material", new FixedMetadataValue(Astra.getPlugin(), goldBlockMaterial.name()));
                    player.sendBlockChange(currentLocation, goldBlockMaterial.createBlockData());
